...
function messageKey(message, index) {
  return message.id ?? `${message.type || 'message'}-${message.sentAt || ''}-${index}`;
}
...
export default function GlobalMemberMessagePanel({
  ...
  const canSend = replyValue.trim().length > 0 && !sendLoading && Boolean(selectedRoomId);

  const partnerRoleLabel =
    currentMemberType === 'BUSINESS' ? '개인회원' : currentMemberType === 'INDIVIDUAL' ? '사업자회원' : '회원';
    ...
    >
      ...
      {!isLoggedIn ? (
        <div className="px-6 py-10 text-center bg-white">
          ...
          <h3 className="text-lg font-bold text-on-surface mb-2">로그인이 필요합니다</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[170px_1fr] h-[460px]">
          <aside className="border-r border-outline bg-[#FAFAFA] overflow-y-auto">
            {roomsLoading ? (
              <div className="px-4 py-6 text-sm text-on-surface-variant">메시지 목록을 불러오는 중입니다...</div>
            ) : rooms.length === 0 ? (
              <div className="px-4 py-6 text-sm text-on-surface-variant">아직 시작한 대화가 없습니다.</div>
            ) : (
              rooms.map((room) => {
                const isActive = room.roomId === selectedRoomId;
                const unread = Number(room.unreadCount || 0);
                const displayName = room.displayName || `회원 #${room.partnerUserId ?? '-'}`;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] text-on-surface-variant">{formatRoomTime(room.lastMessageAt)}</span>
                    </div>
            ...
            {selectedRoom ? (
              <>
                <div className="px-3 py-2 border-b border-outline bg-white">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-bold text-on-surface">{selectedRoom.displayName}</p>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto px-3 py-3 space-y-2 bg-white">
                  {hasMore && (
                    <div className="flex justify-center mb-2">
                      ...
                      >
                        {loadingMore ? '이전 메시지 불러오는 중...' : '이전 메시지 더보기'}
                      </button>
                    </div>
                  )}

                  {messagesLoading ? (
                    <p className="text-sm text-on-surface-variant text-center py-6">메시지를 불러오는 중입니다...</p>
                  ) : messages.length === 0 ? (
                    <p className="text-sm text-on-surface-variant text-center py-6">첫 메시지를 보내 대화를 시작해보세요.</p>
                  ) : (
                    messages.map((message, index) => {
                      const mine = message.senderId === currentMemberId && message.type === 'TALK';
                      const systemMessage = message.type === 'ENTER' || message.type === 'LEAVE';

                      if (systemMessage) {
                        return (
                          <div key={messageKey(message, index)} className="flex justify-center">
                      ...
                      return (
                        <div key={messageKey(message, index)} className={mine ? 'flex justify-end' : 'flex justify-start'}>
                          <div
                            className={[
                ...
                {memberError && (
                  <p className="px-3 pb-3 text-[11px] text-red-600 bg-white">{memberError}</p>
                )}
              </>
            ) : (
